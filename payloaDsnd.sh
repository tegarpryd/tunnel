...
echo -e "${cor[1]} $bar ${cor[0]}"
read -p " Digite o Proxy/Dropbear: " hostprox
...
read -p " Digite o Proxy/ou Dropbear: " hostprox
read -p " Digite a Porta: " portx
echo -e "${cor[1]} $bar ${cor[0]}"
break
;;
...
read -p " Digite o Tempo De Espera! (Segundos): " VARS
echo -e "${cor[1]} $bar ${cor[0]}\n"
...
echo -ne "${cor[1]}Resposta: ${cor[2]}" >&2
...
PID=$!
sleep ${VARS}s
kill -SIGINT $PID &>/dev/null && echo -e "Sem Resposta\n"
done
echo -ne "\033[0m"
